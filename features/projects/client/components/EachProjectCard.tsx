...
import { staggerItem } from "@/features/shared/hooks/use-scroll-animation";
...
const EachProjectCard = ({ project }: { project: IProject }) => {
    return (
        <AnimatedImageContainer variants={staggerItem} className="w-full">
            <Link href={`/projects/${project.id}`} className="relative group overflow-hidden cursor-pointer w-full">
